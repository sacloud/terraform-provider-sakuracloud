...
func TestAccSakuraCloudESME_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: resource.ComposeTestCheckFunc(
			testCheckSakuraCloudESMEDestroy,
			testCheckSakuraCloudIconDestroy,
		),
		Steps: []resource.TestStep{
			{
				Config: buildConfigWithArgs(testAccSakuraCloudESME_basic, rand),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraCloudESMEExists(resourceName, &esme),
			...
			{
				Config: buildConfigWithArgs(testAccSakuraCloudESME_update, rand),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraCloudESMEExists(resourceName, &esme),
...
func testCheckSakuraCloudESMEExists(n string, esme *iaas.ESME) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return errors.New("no ESME ID is set")
		}
		...
		if foundESME.ID.String() != rs.Primary.ID {
...
func testCheckSakuraCloudESMEDestroy(s *terraform.State) error {
	client := testAccProvider.Meta().(*APIClient)

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "sakuracloud_esme" {
			continue
		}
		if rs.Primary.ID == "" {
			continue
		...
		if err == nil {
			return errors.New("ESME still exists")
		}
	}
...
func TestAccImportSakuraCloudESME_basic(t *testing.T) {
	rand := randomName()
	...
	checkFn := func(s []*terraform.InstanceState) error {
		...
		return stateNotEmptyMulti(s[0], "icon_id", "send_message_with_generated_otp_api_url", "send_message_with_inputted_otp_api_url")
	}
...
var testAccSakuraCloudESME_basic = `
...
var testAccSakuraCloudESME_update = `
resource "sakuracloud_esme" "foobar" {
  name           = "{{ .arg0 }}-upd"
