...
func TestAccSakuraCloudEnhancedDB_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: resource.ComposeTestCheckFunc(
			testCheckSakuraCloudEnhancedDBDestroy,
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraCloudEnhancedDBExists(resourceName, &reg),
			...
			{
				Config: buildConfigWithArgs(testAccSakuraCloudEnhancedDB_update, rand, databaseName, password),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraCloudEnhancedDBExists(resourceName, &reg),
...
func testCheckSakuraCloudEnhancedDBDestroy(s *terraform.State) error {
	client := testAccProvider.Meta().(*APIClient)

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "sakuracloud_enhanced_db" {
			continue
		}
		if rs.Primary.ID == "" {
			continue
