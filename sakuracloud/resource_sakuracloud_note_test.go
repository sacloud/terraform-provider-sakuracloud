		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraCloudNoteExists(resourceName, &note),
			...
			{
				Config: buildConfigWithArgs(testAccSakuraCloudNote_update, rand),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraCloudNoteExists(resourceName, &note),
...
func TestAccSakuraCloudNote_withYAML(t *testing.T) {
	resourceName := "sakuracloud_note.foobar"
	name := randomName()
...
const testAccSakuraCloudNote_basic = `
...
const testAccSakuraCloudNote_update = `
resource "sakuracloud_note" "foobar" {
  name    = "{{ .arg0 }}-upd"
  content = "content-upd"
}`

const testAccSakuraCloudNote_yaml = `
resource "sakuracloud_note" "foobar" {
  name    = "{{ .arg0 }}"
  content = "#cloud_init"
