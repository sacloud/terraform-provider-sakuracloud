...
func TestAccSakuraCloudAutoBackup_basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		CheckDestroy: resource.ComposeTestCheckFunc(
			testCheckSakuraCloudAutoBackupDestroy,
			testCheckSakuraCloudDiskDestroy,
			testCheckSakuraCloudIconDestroy,
		),
		Steps: []resource.TestStep{
			{
				Config: buildConfigWithArgs(testAccSakuraCloudAutoBackup_basic, rand),
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "weekdays.0", "fri"),
					resource.TestCheckResourceAttr(resourceName, "weekdays.1", "wed"),
			...
			{
				Config: buildConfigWithArgs(testAccSakuraCloudAutoBackup_update, rand),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraCloudAutoBackupExists(resourceName, &autoBackup),
					resource.TestCheckResourceAttr(resourceName, "name", rand+"-upd"),
					resource.TestCheckResourceAttr(resourceName, "weekdays.#", "2"),
					resource.TestCheckResourceAttr(resourceName, "weekdays.0", "fri"),
					resource.TestCheckResourceAttr(resourceName, "weekdays.1", "sun"),
...
func testCheckSakuraCloudAutoBackupDestroy(s *terraform.State) error {
	client := testAccProvider.Meta().(*APIClient)
...
var testAccSakuraCloudAutoBackup_basic = `
...
var testAccSakuraCloudAutoBackup_update = `
resource "sakuracloud_disk" "foobar" {
  name = "{{ .arg0 }}"
}
