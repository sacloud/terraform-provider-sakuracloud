...
func TestAccSakuraCloudSwitch_basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: buildConfigWithArgs(testAccSakuraCloudSwitch_basic, rand),
			...
			{
				Config: buildConfigWithArgs(testAccSakuraCloudSwitch_update, rand),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraCloudSwitchExists(resourceName, &sw),
...
func testCheckSakuraCloudSwitchDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		zone := rs.Primary.Attributes["zone"]
		_, err := swOp.Read(context.Background(), zone, sakuraCloudID(rs.Primary.ID))
		if err == nil {
