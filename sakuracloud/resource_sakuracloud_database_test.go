...
func TestAccSakuraCloudDatabase_withContinuousBackup(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: buildConfigWithArgs(testAccSakuraCloudDatabase_withContinuousBackup, rand, password),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraCloudDatabaseExists(resourceName, &database),
					resource.TestCheckResourceAttr(resourceName, "name", rand),
...
func TestAccSakuraCloudDatabase_withDiskEncryption(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: resource.ComposeTestCheckFunc(
			testCheckSakuraCloudDatabaseDestroy,
			testCheckSakuraCloudSwitchDestroy,
			testCheckSakuraCloudKMSDestroy,
		),
		Steps: []resource.TestStep{
			{
				Config: buildConfigWithArgs(testAccSakuraCloudDatabase_withDiskEncryption, rand, password),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraCloudDatabaseExists(resourceName, &database),
					resource.TestCheckResourceAttr(resourceName, "name", rand),
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				ImportStateVerifyIgnore: []string{"password", "replica_password"},
			},
		},
	})
}
...
var testAccSakuraCloudDatabase_withContinuousBackup = `
resource "sakuracloud_switch" "foobar" {
  name = "{{ .arg0 }}"
}
...
var testAccSakuraCloudDatabase_withDiskEncryption = `
resource "sakuracloud_switch" "foobar" {
  name = "{{ .arg0 }}"
}
