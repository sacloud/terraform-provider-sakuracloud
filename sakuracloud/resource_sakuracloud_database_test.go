		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "backup.0.weekdays.#", "2"),
					resource.TestCheckResourceAttr(resourceName, "backup.0.weekdays.0", "sat"),
					resource.TestCheckResourceAttr(resourceName, "backup.0.weekdays.1", "sun"),
...
func TestAccSakuraCloudDatabase_ExtraArgs(t *testing.T) {
	resourceName := "sakuracloud_database.foobar"
	rand := randomName()
	password := randomPassword()
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: buildConfigWithArgs(testAccSakuraCloudDatabase_ExtraArgs, rand, password),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraCloudDatabaseExists(resourceName, &database),
					testCheckSakuraCloudDatabaseIsMaster(true, &database),
					resource.TestCheckResourceAttr(resourceName, "database_type", "mariadb"),
					...
					resource.TestCheckResourceAttr(resourceName, "password", password),
					resource.TestCheckResourceAttr(resourceName, "replica_password", password),
					resource.TestCheckResourceAttr(resourceName, "webui", "true"),
					resource.TestCheckResourceAttr(resourceName, "transparent_encryption", "true"),
					resource.TestCheckResourceAttrWith(resourceName, "cluster", func(value string) error {
						if value != "" {
							return nil
						}
						return fmt.Errorf("cluster block: expected not null, got %q", "null")
					}),
					resource.TestCheckResourceAttr(resourceName, "network_interface.#", "1"),
					resource.TestCheckResourceAttr(resourceName, "network_interface.0.ip_address", "192.168.110.101"),
					resource.TestCheckResourceAttr(resourceName, "network_interface.0.netmask", "24"),
...
const testAccSakuraCloudDatabase_ExtraArgs = `
resource "sakuracloud_switch" "foobar" {
  name = "{{ .arg0 }}"
}
...
resource "sakuracloud_database" "foobar" {
  ...
  cluster {
    vip = "192.168.110.200"
    secondary_ip_address = "192.168.110.102"
  }
