...
func TestAccSakuraCloudCDROM_textContent(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: buildConfigWithArgs(testAccSakuraCloudCDROM_textContentUpdate, rand),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "name", rand+"-upd"),
				),
			},
...
func testCheckSakuraCloudCDROMExists(n string, cdrom *sacloud.CDROM) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if foundCDROM.ID.String() != rs.Primary.ID {
			return fmt.Errorf("resource CDROM[%s] not found", rs.Primary.ID)
		}
...
func testCheckSakuraCloudCDROMDestroy(s *terraform.State) error {
	client := testAccProvider.Meta().(*APIClient)
...
var testAccSakuraCloudCDROM_textContent = `
resource "sakuracloud_cdrom" "foobar" {
  name    = "{{ .arg0 }}"
  content = file("test/dummy.json")
}
`
var testAccSakuraCloudCDROM_textContentUpdate = `
resource "sakuracloud_cdrom" "foobar" {
  name    = "{{ .arg0 }}-upd"
